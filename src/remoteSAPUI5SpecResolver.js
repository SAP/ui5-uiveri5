...
RemoteSpecResolver.prototype.resolve = function() {
  ...
  var sSpecFolder = (this.config.remoteSAPUI5SpecResolver ? this.config.remoteSAPUI5SpecResolver.specsFolder : false) || SPECS_FOLDER;
  var sLibsFilter = (this.config.remoteSAPUI5SpecResolver ? this.config.remoteSAPUI5SpecResolver.suitesFolder : false) || "*";
	...
	oPaths = getLibNames(oAppInfo);
	
	//write the suite files with given paths to given folder
	downloadFiles(oPaths, oPaths.targetFolder);
   ...
   * */
    ...
    for (var i = 0; i < aSuiteFiles.specName.length; i++) {
      ...
      for (var j = 0; j < aSuiteFiles.specName[i].length; j++) {
        ...
        downloadFiles([{pathUrl: oSpec.specUri}], specLibFolderName + "/");
