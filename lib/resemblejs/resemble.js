...
_this['resemble'] = function( fileData ){
  ...
  function triggerDataUpdate(){
    updateCallbackArray.forEach(function (updateCallback) {
      updateCallback(data, error);
    });
  }
  ...
  function loadImageData(fileData, callback) {
    ...
    if (Buffer.isBuffer(fileData)) {
      png.parse(fileData, function (err, data) {
        if (err) {
          error = 'Error while parsing image buffer: ' + err.stack;
        } else {
      ...
      fs.createReadStream(fileData)
        ...
        .on('parsed', function () {
          ...
          error = 'Error while parsing image ' + fileData + ': ' + err.stack;
        });
    };
  }
  ...
  function getCompareApi(param){
    ...
    var self = {
      ignoreNothing: function(threshold){
        ...
        tolerance.red = threshold && threshold.red ? threshold.red : 8;
        tolerance.green = threshold && threshold.green ? threshold.green : 8;
        tolerance.blue = threshold && threshold.blue ? threshold.blue : 8;
        tolerance.alpha = threshold && threshold.alpha ? threshold.alpha : 8;
        tolerance.minBrightness = threshold && threshold.minBrightness ? threshold.minBrightness : 16;
        tolerance.maxBrightness = threshold && threshold.maxBrightness ? threshold.maxBrightness : 240;
