...
function Runner(config, connectionProvider, plugins) {
  ...
  if (config.nodeDebug) {
    process['_debugProcess'](process.pid);
    var flow = selenium_webdriver.promise.controlFlow();
...
Runner.prototype.createBrowser = function (plugins, parentBrowser) {
  ...
  var browser_ = new browser.ProtractorBrowser(driver, initProperties.baseUrl, initProperties.rootElement, initProperties.untrackOutstandingTimeouts, blockingProxyUrl);
...
Runner.prototype.run = function () {
  ...
  return q(this.ready_)
    ...
    .then(() => {
      ...
      // 8) var plugins do final cleanup
    })
    .then(() => {
